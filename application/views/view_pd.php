...
<div class="container-fluid">
                    ...
                    <div class="dropdown no-arrow">
                        ...
                        <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in"
                            ...
                            <a class="dropdown-item" id="btnModalTambah" href="#modalForm" data-target="#modalForm"
                                data-toggle="modal">Tambah Peserta Didik</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalCenterTitle">Tambah Data Peserta Didik</h5>
                    ...
                    <form action="" method="POST" id="dataForm">
                        <div class="form-group">
                            ...
                            <input type="text" id="nomor_induk" name="nomor_induk" class="form-control"
                                placeholder="Nomor Induk, contoh 19299299">
                        </div>
                        <div class="form-group">
                            <label>Nama</label>
                            <input type="text" id="nama" name="nama" class="form-control"
                                placeholder="Nama, contoh Afgan">
                        </div>
                        <div class="form-group">
                            <label><strong>Nama Guru :</strong></label>
                            <select name="id_guru" class="form-control select2" id="id_guru">
                            </select>
                        </div>
                        <div class="form-group">
    ...
    function getGuru() {
        $.getJSON('<?= base_url("GetData/getGuru"); ?>', function(data) {
            $('#id_guru').append('<option value="">-- Pilih Guru --</option>');
            $.each(data, function(key, value) {
            $('#id_guru').append('<option value="' + value.id_guru + '">' + value.nama_guru + '</option>');
            });
        });
    }
    ...
    function FormTambah() {
        $('#updateData').hide();
        $('#insertData').show();
        $('.modal-title').html('Tambah Data Peserta Didik');
    ...
    function FormEdit() {
        ...
        $('#insertData').hide();
        $('#updateData').show();
        $('.modal-title').html('Edit Data Peserta Didik');
        $('.modal-title').addClass('text-white');
    ...
    function loadTable() {
        $('#tableAjax').DataTable({
            ...
            destroy: true,
            ajax: {
                url: '<?= base_url("PesertaDidik/loadTable"); ?>'
            },
            columns: [{
                    name: 'id_pd',
                    searchable: false,
    ...
    function showData(id) {
        FormEdit();
        $.ajax({
            url: '<?= base_url("PesertaDidik/showData/"); ?>' + id,
            dataType: 'JSON',
            success: function(result) {
                $('#nomor_induk').val(result.nomor_induk);
                $('#nama').val(result.nama);
                $('#id_guru').val(result.id_guru).trigger('change');
                $('#jk').val(result.jk).trigger('change');
    ...
    function insertData() {
        if ($('#nomor_induk').val() == '') {
            Swal.fire('Ooppss!!', 'Mohon mengisi nomor induk!', 'warning');
        } else if ($('#nama').val() == '') {
            Swal.fire('Ooppss!!', 'Mohon mengisi nama peserta didik!', 'warning');
        } else if ($('#nama_guru').val() == '') {
            Swal.fire('Ooppss!!', 'Mohon mengisi nama guru!', 'warning');
        }
        else if ($('#jk').val() == '') {
            Swal.fire('Ooppss!!', 'Mohon mengisi jenis kelamin!', 'warning');
        }
        else {
            $.ajax({
                url: '<?= base_url("PesertaDidik/insertData"); ?>',
                type: 'POST',
                dataType: 'JSON',
                ...
                success: function(result) {
                    if (result.ping == 200) {
                        keTable();
                        loadTable();
                        toastr.success('Data peserta didik telah ditambahkan!', 'Created!!', {
    ...
    function updateData(id) {
        if ($('#nomor_induk').val() == '') {
            ...
            $.ajax({
                url: '<?= base_url("PesertaDidik/updateData/"); ?>' + id,
                type: 'POST',
                dataType: 'JSON',
                data: $('#dataForm').serialize(),
                success: function(result) {
                    if (result.ping == 200) {
                        keTable();
                        loadTable();
                        toastr.success('Data peserta didik telah diupdate!', 'Updated!!', {
                            showMethod: 'slideDown',
                            hideMethod: 'slideUp',
                            timeOut: 2000
                        });
    ...
    function deleted(id) {
        Swal.fire({
            ...
            text: 'Penghapusan data peserta didik!',
            type: 'warning',
            ...
            if (result.value) {
                $.ajax({
                    url: '<?= base_url("PesertaDidik/deleted/"); ?>' + id,
                    dataType: 'JSON',
                    success: function(result) {
                        if (result.ping == 200) {
                            loadTable();
